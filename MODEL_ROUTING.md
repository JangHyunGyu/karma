...
```toml
KARMA_TEXT_MODEL_PRESET = "openrouter-gemma"
...
the exact OpenRouter `session_id`. Karma's Gemma route lists DeepInfra Turbo
and Venice together, so the Venice-only `prompt_cache_key` is not sent. D1
`perf_stats` records the caller key, actual model, provider route, and
available cache token metrics.
