...
function createLocationSelect(containerId) {
  ...
  tipBox.dataset.ko = '사주는 태어난 순간의 천체 위치를 기준으로 합니다. 같은 시각이라도 나라마다 시간대(타임존)가 다르기 때문에, 해외 출생자는 태어난 지역을 선택해야 정확한 사주를 볼 수 있습니다. 목록에 내 나라가 없으면 같은 UTC 시간대의 다른 지역을 선택하세요. 한국 출생이면 그대로 두시면 됩니다.';
  tipBox.dataset.en = 'Saju is based on the celestial positions at the moment of birth. Since time zones differ by country, selecting your birth location ensures accurate calculation. If your country is not listed, choose another region with the same UTC offset. If born in Korea, leave the default.';
