...
아래 순서대로 먼저 관찰한 뒤 해석하세요. 실제 전문가 감수 데이터가 아니라, 전통 관상에서 흔히 보는 판독 기준을 구조화한 것입니다.

1. 사진 품질: 정면성, 초점, 조명, 얼굴 가림, 표정 과장, 필터/보정 여부를 먼저 판정. 품질이 낮으면 해당 부위 점수를 낮추고 "사진상 확인 어려움"이라고 명시.
2. 얼굴형/삼정: 이마-중안부-하관 비율, 얼굴이 긴지 둥근지 각진지, 좌우 균형을 본 뒤 초년/중년/말년 흐름으로 연결.
3. 이마: 높이, 폭, 헤어라인, 굴곡, 가림 여부를 보고 학습력·초년 지원·계획성을 해석. 머리카락에 가려지면 추정 금지.
4. 눈/눈썹: 눈꼬리 방향, 눈빛 선명도, 눈썹 간격·농도·정돈감, 좌우 차이를 보고 대인관계·감정 기복·경계심을 해석.
5. 코/광대: 콧대, 코끝, 콧방울, 광대 돌출과 균형을 보고 자존심·재물 관리·사회적 추진력을 해석.
6. 입/턱: 입술 두께, 입꼬리 방향, 턱 길이와 폭, 하관 안정감을 보고 언변·관계 지속력·말년 기반을 해석.
7. 관찰→전통 해석→현실 조언 순서로 쓰세요. 관찰 근거 없이 성격이나 운세만 말하면 실패입니다.
8. 건강은 의학적 진단처럼 쓰지 말고, 얼굴에 보이는 피로감·긴장감·생활관리 주의 정도로만 표현하세요. 특정 질병 확정, 발병 나이 단정은 금지.
9. 닮은 유명인은 실제 인물 식별처럼 단정하지 말고 "분위기상 참고" 수준으로만 쓰세요. 확신이 낮으면 빈 문자열로 두세요.`;
}
