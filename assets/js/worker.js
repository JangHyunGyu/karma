...
async function callGemini(apiKeys, prompt, _caller, _env) {
  ...
  const body = JSON.stringify({
    ...
    generationConfig: { maxOutputTokens: 4096, temperature: 0.5, thinkingConfig: { thinkingLevel: "high" } },
  });
  const errors = [];
...
async function callGeminiVision(apiKeys, prompt, imageBase64, mimeType, _env) {
  const body = JSON.stringify({
    ...
    generationConfig: { maxOutputTokens: 8192, temperature: 0.5, thinkingConfig: { thinkingLevel: "high" } },
  });
  const errors = [];
  for (let i = 0; i < apiKeys.length; i++) {
    const isLast = i === apiKeys.length - 1;
...
### 1. 종합 궁합 (성격/관계)
- 위 합/충 데이터를 반드시 반영하여 해석
- 두 일간의 오행 관계가 실제 관계에 미치는 영향
- 오행 과다/부족을 서로 보완하는지 여부

### 2. 속궁합 (에너지 교감/신체적 궁합)
사주명리학 원리에 기반한 두 사람의 에너지 교감과 신체적 조화를 해석하세요.
- 음양 조화: ${yinYangMatch ? '양+음 조합으로 이상적' : '동일 음양으로 에너지 균형 필요'}
- 수(水)·화(火) 오행 비율: A(수${sajuA.ohangCount.수}/화${sajuA.ohangCount.화}), B(수${sajuB.ohangCount.수}/화${sajuB.ohangCount.화}) — 수는 감성/본능, 화는 열정/표현
...
- 재성(財星) 관계: 일간이 극(剋)하는 오행이 재성 → A의 재성=${SANGGEUK[sajuA.ilganOhang]}, B의 재성=${SANGGEUK[sajuB.ilganOhang]}
- 토(土)·금(金) 오행 비율: A(토${sajuA.ohangCount.토}/금${sajuA.ohangCount.금}), B(토${sajuB.ohangCount.토}/금${sajuB.ohangCount.금}) — 토는 안정/저축, 금은 실행/결실
